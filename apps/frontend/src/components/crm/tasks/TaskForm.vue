<template>
  <div class="bg-card rounded-lg p-6 w-full max-w-md relative z-10">
    ...
    <FormControl label="Título da Tarefa" forLabel="taskTitle">
      <BaseInput v-model="form.title" id="taskTitle" placeholder="Digite o título da tarefa" />
    </FormControl>

    <FormControl label="Data Prevista" forLabel="taskDueDate" class="mt-4">
      <BaseInput v-model="form.due_date" id="taskDueDate" type="date" />
    </FormControl>

    <div class="flex justify-end gap-3 mt-6">
      <DefaultButton variant="default" size="sm" @click="$emit('cancel')"> Cancelar </DefaultButton>
      <DefaultButton
        variant="primary"
        size="sm"
...
watch(
  () => props.title,
  (newTitle) => (form.value.title = newTitle),
);
watch(
  () => props.dueDate,
  (newDueDate) => (form.value.due_date = newDueDate),
);
