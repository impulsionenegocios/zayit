...
export function useFileUpload(options = {}) {
    const { multiple = false, compress = true, delayAfterUpload = 2000, autoUpload = false, uploadUrl = '', uploadFieldName = 'file', accept, validateFileType: customValidateFileType, } = options;
    const files = ref([]);
    const blobUrls = ref([]);
    ...
    function validateFileType(file) {
        ...
        if (!accept)
            return true; // Sem restrição se não houver
    ...
    async function handleFileChange(event) {
        if (!canUpload.value || isLoading.value)
            return;
    ...
    async function handleDrop(event) {
        if (!event.dataTransfer?.files || !canUpload.value || isLoading.value)
            return;
